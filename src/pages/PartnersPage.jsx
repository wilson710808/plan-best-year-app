...
const PARTNER_PROFILES = [
  { id: 'p1', name: '小明', role: 'traveler', emoji: '🚶', greeting: '你好！我也是正在努力的人，一起加油吧！', promptSuffix: '你是一位同行者伙伴，和用戶一起成長，分享自己的掙扎和進步。使用繁體中文。' },
  { id: 'p2', name: '老張', role: 'veteran', emoji: '🧓', greeting: '我走過這條路，有什麼想問的嗎？', promptSuffix: '你是一位過來人伙伴，已經成功走過這條路，分享經驗和建議。使用繁體中文。' },
  { id: 'p3', name: '小美', role: 'newbie', emoji: '🌸', greeting: '你好～我剛開始，請多多指教！', promptSuffix: '你是一位新手伙伴，剛開始旅程，常需要鼓勵但有新鮮視角。使用繁體中文。' },
  { id: 'p4', name: '陳教練', role: 'coach', emoji: '🎯', greeting: '你好，我是你的專業教練，有什麼想討論的？', promptSuffix: '你是一位專業教練伙伴，提供結構化指導和專業建議。使用繁體中文。' }
]

export default function PartnersPage({ navigate }) {
  const { t, userName, partners, aiConversations, setAiConversations } = useApp()
  const { query } = useAI()
  ...
  const sendMessage = async () => {
    ...
    const systemPrompt = activePartner.promptSuffix + (dbPartner.missedToday ? `\n注意：你今天忘記打卡了，承認這一點並說明天會補上。` : '')
